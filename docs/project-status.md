...
## 2026-08-05：室内环境与天气变化主动播报

- 环境空间新增可选的室内传感器绑定，真实设备报告进入后会评估同空间的室内温度、湿度、气压和光照阈值；主播报设备可绑定为 `xiaozhi-F02254`。
- 室外规则新增近一小时降水阈值和天气文字变化。降水触发会提醒携带雨具；室内温度触发会建议开启空调或通风。所有播报仍须满足连续命中、冷却、静音和动态 TTS 已启用条件。
- Flyway V13 仅为 `environment_spaces` 增加 `indoor_sensor_device_id` 外键和索引；不修改既有采集数据、天气读数或播报任务。后端 45 项测试通过（2 项 MySQL 集成测试按未提供数据库跳过），前端生产构建通过。
- 小智待机屏显示室内状态需要匹配实际板型后修改其固件，尚未生成小智固件或执行烧录。

## 2026-08-04：环境监测设备遥测优化

- 环境监测 ESP32 已实机确认 MQTT QoS 1 发布排队并收到 Broker `PUBACK`；采样周期保持 5 秒、默认上报周期保持 60 秒。
- IoT 新增 Flyway V12，为 `device_reports` 增加 `pressure`（hPa）与 `illuminance`（lux）。MQTT/HTTP 上报、设备详情最近读数、分页采集表和趋势图均透传这两项；趋势卡只显示实际有读数的指标。
- 设备端改为以结构化 JSON 发送这两项，不再写入 `message`；正常串口采样降为 DEBUG，仅在周期上报时输出一条包含全部读数的 INFO 日志，减少无效刷屏。
- ESP-IDF 5.5.4 已完成环境监测项目全量构建，`environment_monitor.bin` 为 `0xfe200`，3 MiB factory 分区剩余 `0x201e00`（67%）。用户已确认固件烧录完成；烧录不擦除保存的 Wi-Fi/MQTT NVS 配置。
- 本机远程 HiveMQ Compose 仅作构建与迁移验证：复用本机 `aiot-log-system-remote_mysql-data` 后，Flyway 从既有 V2 顺序执行至 V12，未删除数据卷；本机验证设备 `ENV-MONITOR-001` 的旧固件上报可入库。该结果不代表 NAS 已升级，NAS 仍需导入本次离线镜像包；旧固件的气压/光照仍在 `message`，待 COM8 释放并烧录新固件后才会进入结构化列。

## 2026-08-01：环境天气条件播报（源码完成，待私有服务与实机验收）

- 环境空间可配置详细地址、坐标和主小智设备；V9 保存真实室外天气/AQI读数，V10 保存规则及逐次判定/投递审计事件。
- 环境监测页支持编辑既有空间，避免因初建时未填写坐标而必须删除重建；天气服务即使已配置，未填写完整经纬度的空间仍明确禁止刷新。
- 天气服务凭证改为环境监测页面可视化维护：Host、JWT kid、项目 ID 与 PEM 私钥保存到后端私有目录，私钥只接收不回显，不写入数据库或仓库。远程 Compose 已补齐 `IOT_CONFIG_DIR=/app/private-config`，确保页面配置真正写入挂载的 `docker/local` 并跨容器重建保留。
- 规则可按温度或 AQI 的绝对阈值或相邻读数变化幅度触发，并要求连续命中；支持冷却和跨午夜静音时段。未命中会记录 `CLEAR` 以打断连续计数；未满足连续次数记录 `OBSERVED`，其他禁止投递条件记录 `SUPPRESSED`。
- 自动播报只在空间已启用、已绑定有效设备、动态 TTS 已显式启用且不处于静音/冷却期时进行。TTS 未配置或失败不会改发固定测试 Opus；失败仅保存 `FAILED` 事件。
- 已完成后端 JDK 17 Maven 42 项测试（2 项真实 MySQL 迁移测试因本机未配置而跳过）以及前端 TypeScript/Vite 生产构建。私有离线包 `dist/aiot-remote-mqtt-v1.2.8-images.tar` 已构建，SHA-256 为 `f595c27fc5e2bb578ee0e8a80babf541f051027c709eb9985fb516156b3a9884`；尚未用真实私有天气凭证、动态 TTS 和小智设备进行端到端验收。
- 私有离线包 `dist/aiot-remote-mqtt-v1.3.4-images.tar` 已构建，SHA-256 为 `d031e330715bd3ccf0b501f06ea2ba000ad1309c7b7ab5d78f3f925de9830039`。该包修复远程 Compose 的天气私有配置持久化路径；群晖导入后必须复用整个 `docker/local`，并在环境监测页重新保存一次天气配置后再刷新。
- 天气专属 Host 的 DNS 与 HTTPS 可达性已独立验证；后端已改为遍历安全异常链，避免 DNS、连接、超时或 TLS 被误报为“检查私有配置”。待 `v1.3.5` 群晖升级后，以页面显示的具体安全诊断继续排查。
- `v1.3.6` 已保留天气请求失败的原始 cause，群晖后端日志可显示 `Caused by:` 根因；页面继续只返回脱敏诊断。
- 已定位和风成功响应使用 gzip 压缩而旧客户端直接按 JSON 解析的问题；`v1.3.7` 自动解压后再解析，待群晖导入后验证真实天气/AQI 入库。
- 群晖 `8080` 已被宿主机服务占用；远程 Compose 已移除无必要的后端宿主机端口映射，前端继续通过内部网络调用后端。
- 群晖 `80` 同样被宿主机服务占用；远程前端默认发布到可覆盖的 `WEB_PORT`，默认 `18080`。
- 后续群晖升级统一执行部署预检：以 Container Manager 的 YAML 配置为准，检查镜像标签、端口、私有配置目录和持久化文件；错误反馈必须包含安全的出错阶段与位置，不再只给泛化提示。

## 2026-08-01：小智运行日志刷新延迟优化（源码完成，待实机验收）

- 2026-08-02 新版小智同时上报 `startup` 与 `firmware_started` 会触发旧版双启动边界，导致一次启动被拆成多条日志。现仅 `startup` 新建批次，`firmware_started` 合并到当前启动批次；已新增回归用例。私有离线包 `dist/aiot-remote-mqtt-v1.3.9-images.tar` 已构建，SHA-256 为 `AF87AB4BA931D53803995112F12D1D8ED6BC3BA49B487F2B6AE234C23CD5A12A`，待群晖验收。
- 截图继续显示同秒“1 条 + 3 条”后，进一步定位为新版固件可能迟到上报 `startup`。源码已增加 2 秒乱序容忍：窗口内该事件并入当前批次，窗口外仍新建批次；该补充修复已构建为 `v1.4.0` 离线镜像包。
- `Firmware startup completed` 已纳入后端写入和前端历史记录的中文映射，统一显示为“固件启动完成”。`dist/aiot-remote-mqtt-v1.4.0-images.tar` 的 SHA-256 为 `B8FCFA8EEF25423305E52B8E2C8988356902252FE09DC5BCB54B5E6F701CF9D1`，待群晖验收。
- 设备工作台和全局日志页的可见页面刷新间隔由 1 秒收紧为 0.5 秒，目标是在 MQTT 消息已入库后 0.5 秒内显示在页面。
- 保留请求互斥和后台暂停；若上一次查询尚未结束，控制器会在其结束后最多 50 毫秒再次检查并刷新，避免慢响应导致连续跳过周期、实际体验退化为数秒。
- 已新增该补偿行为的前端单元测试；环境监测完成后已统一构建 `v1.4.0` 离线镜像包，待导入群晖并以真实小智启动日志确认端到端延迟。
- 2026-08-02 设备详情的 MCP 操作记录查询已从设备档案、零件和遥测的串行刷新链路拆出；进入 MCP 页签立即加载，页签可见时以 0.5 秒刷新。前端测试和生产构建通过。若小智“待命”后日志仍约 5 秒才出现，则延迟位于固件事件发布或 MQTT 入库之前，需以设备/桥接器时间戳继续定位。

## 2026-07-29：设备档案与零件清单（源码完成，待部署验收）

- 新增设备档案字段：厂商、型号、序列号、固件版本。
...
- `v1.2.1` 已完成群晖部署并验证提醒创建与后端发布，但因 UTC manifest 缺陷设备未播放；`v1.2.3` 已导入群晖并完成固定直发验收：时间窗口为正、35 个 Opus 帧均被处理、收到 `received/played` ACK，扬声器实际播放固定中文语音。桥接器默认设备绑定已部署并完成官方小智“一分钟后提醒”实机验收；当前提醒仍播放固定中文测试语音。下一步是以真实 TTS 资源按提醒文本生成裸 Opus 帧，再复验动态播报。桥接器和固件后续改动必须保持该已验收链路。
...
- 提醒管理前端已完成源码实现，并已构建私有离线 `v1.2.5-remote-mqtt` 镜像包：全局页面可按设备编号筛选，设备工作台增加提醒页签；展示计划/实际触发时间、发布任务和状态，且仅允许取消 `SCHEDULED` 提醒。复用既有 `GET /api/reminders` 与 `DELETE /api/reminders/{id}`，不改变提醒创建、TTS、MQTT 或 ACK 逻辑；尚未导入或部署群晖。

- 项目主页 README 已补充远程提醒、动态 Piper TTS、设备 ACK、提醒管理和 `v1.2.3` 公开镜像与私有离线 `v1.2.4` 的边界说明；未写入任何私有网络地址、凭证、Token 或真实设备数据。

- 前端全页面自动刷新已完成源码实现：普通页面在可见时每 5 秒刷新，日志页保留 1 秒；首页、设备、设备详情/AI MCP、提醒、告警规则、标签和 MQTT 状态在重新可见、窗口聚焦、网络恢复或浏览器恢复时立即刷新。自动刷新不显示加载遮罩、后台不发起请求，并通过请求互斥避免重复调用。

- 私有离线 `v1.2.6-remote-mqtt` 镜像包已构建并导出，待人工导入群晖；`v1.2.5` 保留为提醒管理页面版本，未被覆盖。
...
- 新增 Flyway V6 与 `/api/mcp-tool-executions`，设备工作台新增“AI MCP 操作”页签显示全局脱敏记录。小智 `feature/aiot-mcp-reminder-integration` 整合分支的群晖桥接器已实现 IoT 提醒工具和 Home Assistant/PC MCP 的透明转发审计；IoT API 路径与字段已完成本地契约核对，尚未部署群晖、填入私有 `IOT_API_URL` 或联调官方 MCP。
...
环境天气播报不与刷新延迟优化同包发布：V8～V10 已进入源码并通过常规回归，但尚未构建或部署包含这些迁移的远程镜像；应在真实私有天气、动态 TTS 和小智投递验收通过后再统一构建离线包。
...
- 已将环境监测从摘要扩展为可展开详情：当前天气、温湿度和 AQI 保持在卡片主视图；详情展示体感温度、风况、降水、气压、能见度、云量、露点、PM2.5/PM10/NO₂/O₃/SO₂/CO、空气质量等级、健康建议及关联监测站，污染物单位采用数据源实际返回值而非前端猜测。
- 新增 Flyway V11，仅向 `environment_outdoor_readings` 追加可空字段；已有环境空间和历史天气读数不受影响。旧读数详情显示“−”，下一次手动刷新即可写入新字段。
- 页面将原始 ISO 时间改为秒级可读格式；数据仍以天气数据源实际观测/更新延迟为准，不等同于街道实测。
- JDK 17 后端 42 项测试通过（2 项需独立 MySQL 的迁移测试按环境变量跳过），前端 TypeScript 检查与生产构建通过。`v1.3.8` 离线镜像包已生成，SHA-256 为 `4caa8680b1fbbcc0ce78008daebfc3b45b33f523ff23528436393af960797e21`；待群晖导入和页面验收。
