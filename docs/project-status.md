...
1. 更新群晖到 `v1.1.6-remote-mqtt`，继续复用原数据库卷和私有环境文件，并确认后端容器显示 `healthy`、前端在后端健康后启动。
