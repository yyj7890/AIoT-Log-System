...
2026-07-20 工程质量候选修改（拟发布为 `v1.1.6-remote-mqtt`）：后端增加 Spring Boot Actuator，管理端口仅监听 `127.0.0.1:8081`，只公开不含组件细节的 `health` 端点且不映射至宿主机。源码构建的局域网与远程 Compose 增加后端内部健康检查，前端等待 `service_healthy` 后启动；远程 GHCR Compose 已固定到候选 `v1.1.6-remote-mqtt`，仅供标签发布后使用。现有 GHCR `v1.1.5-remote-mqtt`、群晖容器和私有配置未修改。远程前后端镜像构建、JDK 17 Maven 14 项后端测试、Compose 结构校验和隔离容器验收均已通过；健康端点返回 `UP`，同一 Docker 网络中的其他容器不能访问内部 `8081`。本候选修改尚未提交、发布或部署。
...
- GHCR 包应保持 Public；`v1.0.0-lan` 与远程 `v1.1.0` 至 `v1.1.5` 固定标签已发布，镜像签名和漏洞扫描尚未配置。

## 下一步

1. Actuator 候选修改的构建、后端测试和内部端口隔离已通过；获得明确授权后提交并发布新的远程固定版本，再更新群晖并确认后端容器显示 `healthy`、前端在后端健康后启动。
2. 继续进行群晖长时间观察，并在后续后端正常重启时确认系统运行时长从零重新计时；如首次 API 请求失败，确认日志轮询仍能自行恢复。
3. 真实故障/恢复/再次故障状态流转已有单元测试覆盖；后续真机触发时复核 `PENDING`/`RESOLVED`/`PENDING`，并继续确保运行环境只有一个远程订阅后端实例。
4. 局域网 UDP `19830` 回归和 GitHub 脱敏截图按当前安排暂缓，恢复时继续按专项文档验收。
