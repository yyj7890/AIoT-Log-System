...
- 已有 23 项后端测试、前端类型检查和生产构建验证；更完整的自动化回归和 AI 分析尚未开发
...
1. 基于现有23项后端测试，随核心修改逐步补充 MQTT、状态流转、数据库迁移、前端轮询和 Docker 升级回归。
2. 为 GHCR 镜像增加漏洞扫描，评估镜像签名，复核标签规则，验证旧版本回退，并评估国内镜像仓库。
