...
- GHCR 前后端镜像、漏洞门禁、来源证明与 GitHub Release 已成功。腾讯云 TCR 前端同步成功；后端直接跨仓库复制曾收到 HTTP/2 `PROTOCOL_ERROR`。同步工作流已改为 GitHub Runner 本地拉取后推送，并对瞬态传输最多重试三次；等待该工作流验证后才可给出后端国内镜像拉取命令。
