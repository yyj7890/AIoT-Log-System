...
- 动态 TTS 已完成私有网关、文本播报服务和提醒接入；网关返回 Opus 帧后复用既有 manifest、二进制 MQTT 与 ACK。用户已独立部署 Piper 网关并验证私有 HTTP 返回合法 16 kHz/单声道/60 ms Opus 帧；IoT 仍运行 v1.2.3 固定测试语音，待导入 v1.2.4 镜像并通过私有环境变量显式启用。HTTP 连接和读取均受超时限制，网关失败不泄露文本、音频或私密配置。
