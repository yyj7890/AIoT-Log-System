...
- 顶部后端在线探测与 MQTT 跨窗口故障关闭已随 `v1.1.7-remote-mqtt` 发布，但群晖仍运行旧镜像，升级前仍会表现出原问题。

## 下一步

1. 在不删除数据卷和私有配置的前提下，将群晖原地升级到 `v1.1.7-remote-mqtt`。
