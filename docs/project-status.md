...
1. 下一次表结构变化前引入 Flyway 或 Liquibase；没有结构变化时暂不启动。
2. 补充 OpenAPI/Swagger、统一错误码、异常响应和运行日志规范。
3. 基于现有测试，随核心修改逐步补充 MQTT、状态流转、数据库迁移、前端轮询和 Docker 升级回归。
4. 为 GHCR 镜像增加漏洞扫描，评估镜像签名，复核标签规则，验证旧版本回退，并评估国内镜像仓库。
