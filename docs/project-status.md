...
- Docker 中文初始化数据修复
- 既有 Docker 四服务验收完成；最新私有凭证、MQTT 认证与 UDP 自动发现 Docker 配置尚未重新构建验证
- GHCR 成品镜像工作流与部署脚本已准备，尚未成功发布镜像或在新环境完成拉取验证
...
- Docker 安全配置更新尚需执行 `docker-update.cmd` 进行一次干净重建，并在可信局域网验证 MQTT 认证与 UDP `19830` 自动发现。
- GHCR 包首次发布后需要人工设为 Public；尚未准备发布标签、镜像签名或漏洞扫描。

## 下一步

1. 提交并推送 GHCR 工作流，等待 Actions 成功后将两个 GitHub Package 设为 Public；在无 Java/Node/Maven 的环境运行 `docker-ghcr-update.cmd` 验收。
2. 在停止本地开发版后执行 `docker-update.cmd`，验证私有 `.env`/`docker/local/` 自动生成、MQTT 认证和 UDP `19830` 自动发现；不得提交生成文件。
3. 按截图清单准备已脱敏的首页、设备详情、中文运行日志、MQTT 认证状态和批量删除模式截图；上传前执行安全检查清单。
4. 确认固件不会重复发布同一轮启动事件，并持续观察启动汇总日志。
