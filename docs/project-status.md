...
2026-07-21 本地修复完成：`AppHeader` 现在使用 `/api/system/runtime` 作为轻量在线探针，初始显示“后端检测中”，成功显示“后端已接入”，失败清空过期运行时长并显示“后端未连接”，每 5 秒及页面恢复时自动重试。后端收到 MQTT 恢复事件时，会在 30 秒事件汇总之外独立查找并关闭最新对应的未恢复 MQTT 故障；关闭旧故障与写入恢复事件处于同一事务。新增跨窗口恢复测试后，JDK 17 Maven 共 15 项测试全部通过；前端 TypeScript 检查和 Vite 生产构建通过。远程 Compose 已准备固定引用 `v1.1.7-remote-mqtt`，该标签尚待发布，群晖仍运行 `v1.1.6-remote-mqtt`。
...
- 已有 14 项后端测试、前端类型检查和生产构建验证；更完整的自动化回归和 AI 分析尚未开发
...
- 真实硬件长时间持续上报
- 群晖 Container Manager 中停止、启动全过程的 `healthy` 状态变化留证；API、MQTT、前端轮询和运行时长恢复已验证
- 新镜像部署后的跨 30 秒窗口 MQTT 故障关闭与 `PENDING`/`RESOLVED`/`PENDING` 群晖实测
- 更完整的自动化回归测试
...
- GHCR 包应保持 Public；`v1.0.0-lan` 与远程 `v1.1.0` 至 `v1.1.6` 固定标签已发布，镜像签名和漏洞扫描尚未配置。
- 顶部后端在线探测与 MQTT 跨窗口故障关闭已在源码修复并通过构建/测试，但群晖仍运行旧镜像，发布升级前仍会表现出原问题。

## 下一步

1. 发布包含本次修复的新远程固定版本镜像，并在不删除数据卷和私有配置的前提下升级群晖。
2. 在群晖复核离线/恢复、跨窗口 `PENDING`/`RESOLVED`/`PENDING`、QoS 1 重投、相邻去重和 MQTT 处理计数。
3. 下一次表结构变化前引入 Flyway 或 Liquibase；没有结构变化时暂不启动。
4. 补充 OpenAPI/Swagger、统一错误码、异常响应和运行日志规范。
5. 基于现有测试，随核心修改逐步补充 MQTT、状态流转、数据库迁移、前端轮询和 Docker 升级回归。
6. 为 GHCR 镜像增加漏洞扫描，评估镜像签名，复核标签规则，验证群晖拉取式升级、数据卷复用和旧版本回退，并评估国内镜像仓库。
