...
同日上述两项问题已在源码修复并通过 15 项后端测试及前端生产构建；前后端 `v1.1.7-remote-mqtt` 镜像和 GitHub Release 已发布。群晖尚未升级；下一次部署必须继续复用原 MySQL 数据卷和私有环境文件，不得使用 `down -v`，升级后再复测顶部离线状态、跨窗口故障关闭和 Container Manager 的 `healthy` 状态变化。
