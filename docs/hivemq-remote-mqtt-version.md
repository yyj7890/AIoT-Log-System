...
当前状态：**`v1.1.7-remote-mqtt` 已完成源码和发布配置准备，增加后端在线状态探测并修复跨 30 秒汇总窗口的 MQTT 故障关闭，尚待标签发布和群晖升级；群晖当前仍运行 `v1.1.6-remote-mqtt`。真实凭证未写入仓库。**
...
- 两个 GHCR 包以标签区分版本：`v1.0.0-lan` 是固定局域网版，`v1.1.0-remote-mqtt` 是首个远程版，`v1.1.1-remote-mqtt` 增加远程状态中文文案和 QoS 1 相邻重投保护，`v1.1.2-remote-mqtt` 增加 MQTT 状态页自动刷新，`v1.1.3-remote-mqtt` 增加 `WARN` 等级兼容并将状态页和日志管理页统一为 1 秒可靠刷新，`v1.1.4-remote-mqtt` 增加详情实时同步和本地 AI 回退事件中文化，`v1.1.5-remote-mqtt` 增加日志轮询自愈和系统运行时长显示，`v1.1.6-remote-mqtt` 增加后端容器健康检查和前端就绪依赖，`v1.1.7-remote-mqtt` 增加后端在线状态探测并修复跨窗口故障关闭。当前源码远程 GHCR Compose 使用 `v1.1.7-remote-mqtt`，群晖仍运行 `v1.1.6-remote-mqtt`；旧远程标签保留用于回退，避免误用 `latest`；`latest` 只允许 `main` 分支推送更新并保持局域网语义，版本标签事件不得覆盖它。
...
- 当前已完成源码修复并将远程 Compose 固定到 `v1.1.7-remote-mqtt`，尚未推送版本标签或升级群晖；`v1.1.6-remote-mqtt` 仍保留原行为。
