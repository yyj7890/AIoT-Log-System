...
当前状态：**`v1.1.5-remote-mqtt` 已发布，修复日志页首次请求失败后轮询未启动的问题，并增加后端运行时长显示；群晖当前仍运行 `v1.1.4-remote-mqtt`，等待人工升级。后端 Maven 与前端生产构建已通过，真实凭证未写入仓库。**
...
- 两个 GHCR 包以标签区分版本：`v1.0.0-lan` 是固定局域网版，`v1.1.0-remote-mqtt` 是首个远程版，`v1.1.1-remote-mqtt` 增加远程状态中文文案和 QoS 1 相邻重投保护，`v1.1.2-remote-mqtt` 增加 MQTT 状态页自动刷新，`v1.1.3-remote-mqtt` 增加 `WARN` 等级兼容并将状态页和日志管理页统一为 1 秒可靠刷新，`v1.1.4-remote-mqtt` 增加详情实时同步和本地 AI 回退事件中文化，`v1.1.5-remote-mqtt` 增加日志轮询自愈和系统运行时长显示。当前源码远程 Compose 已固定引用 `v1.1.5-remote-mqtt`；群晖当前运行 `v1.1.4-remote-mqtt`，旧远程标签保留用于回退，避免误用 `latest`；`latest` 只允许 `main` 分支推送更新并保持局域网语义，版本标签事件不得覆盖它。
...
- `docker-compose.remote.ghcr.yml` 已固定引用 `v1.1.5-remote-mqtt`；群晖继续运行 `v1.1.4-remote-mqtt`，尚未执行升级。
- 群晖升级后需复现首次请求失败和页面休眠恢复，确认日志无需切换菜单即可自动更新，并检查后端重启后运行时长从零重新开始。
