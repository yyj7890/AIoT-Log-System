...
当前状态：**`v1.1.9-remote-mqtt` 已发布前后端 GHCR 和腾讯云TCR镜像、digest来源证明及GitHub Release；两个TCR公有镜像可匿名解析且顶层digest与GHCR一致。群晖已通过TCR镜像完成保留数据升级和继续写入验收。真实凭证未写入仓库。**
...
| `v1.1.9-remote-mqtt` | **已发布并完成群晖升级验收** | 包含OpenAPI、统一错误追踪、共享轮询控制器及对应回归 | 增加MQTT Topic/Payload一致性校验、Tomcat 10.1.57安全修复及完整回归入口；数据库仍为Flyway V1 | TCR原地升级后原46条日志保留；Swagger/OpenAPI和远程MQTT正常，测试计数`2/2/0`，新增ID47且总数47 |
...
- 两个 GHCR/TCR 包以固定标签区分版本，完整差异和发布/群晖状态统一维护在本文“统一版本变更总表”。当前最新发布并部署的是 `v1.1.9-remote-mqtt`。旧远程标签保留用于回退，避免误用 `latest`；`latest` 只允许 `main` 分支推送更新并保持局域网语义，版本标签事件不得覆盖它。
