...
当前状态：**`v1.1.5-remote-mqtt` 已发布并部署群晖，修复日志页首次请求失败后轮询未启动的问题，并增加后端运行时长显示；首次浏览器与真机验收通过。`v1.1.6-remote-mqtt` 后端容器健康检查候选已完成构建、后端测试和隔离容器验证，但尚未提交、发布或部署。真实凭证未写入仓库。**

### v1.1.6 后端容器健康检查候选（2026-07-20）
...
- 远程 GHCR Compose 已固定引用候选 `v1.1.6-remote-mqtt`，只在同名标签和镜像发布后用于更新；群晖当前运行的 `v1.1.5-remote-mqtt` 不受影响。
