...
当前状态：**`v1.1.9-remote-mqtt` 已发布前后端 GHCR 和腾讯云TCR镜像、digest来源证明及GitHub Release，并完成群晖升级验收。2026-07-24群晖成功回退到`v1.1.8-remote-mqtt`，47条数据保留，测试后新增ID48且总数48；当前运行1.1.8，等待决定是否升回1.1.9。真实凭证未写入仓库。**
...
| `v1.1.8-remote-mqtt` | **已发布；当前为回退验收运行版本** | 无功能变化；不含OpenAPI与新版错误追踪 | 接入 Flyway；空库执行 V1，旧库保留数据并建立 V1基线 | 从1.1.9回退后47条日志保留；MQTT`1/1/0`，新增ID48且总数48 |
| `v1.1.9-remote-mqtt` | **已发布并完成群晖升级验收，后成功回退** | 包含OpenAPI、统一错误追踪、共享轮询控制器及对应回归 | 增加MQTT Topic/Payload一致性校验、Tomcat 10.1.57安全修复及完整回归入口；数据库仍为Flyway V1 | TCR原地升级后原46条日志保留；测试新增ID47；随后直接回退1.1.8成功 |
...
- 两个 GHCR/TCR 包以固定标签区分版本，完整差异和发布/群晖状态统一维护在本文“统一版本变更总表”。最新发布版本是`v1.1.9-remote-mqtt`；群晖完成升级与回退验证后当前运行`v1.1.8-remote-mqtt`。旧远程标签保留用于回退，避免误用 `latest`；`latest` 只允许 `main` 分支推送更新并保持局域网语义，版本标签事件不得覆盖它。
