...
当前状态：**`v1.2.0-remote-mqtt` 已发布前后端 GHCR 和腾讯云TCR镜像、digest来源证明及GitHub Release，并完成群晖实机验收。此前1.1.9→1.1.8→1.1.9双向演练确认升级、回退和再升级均可继续写入；1.2.0确认原数据保留、设备独立日志自动刷新、页面保存凭据立即重连及后端重启后持续生效。真实凭证未写入仓库。**
