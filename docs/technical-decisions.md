...
最终选择：数值 `code` 继续与 HTTP 状态一致以兼容现有前端，新增枚举名形式的稳定 `errorCode` 用于程序判断，`message` 只用于用户显示。业务层只能抛出集中 `ErrorCode`，不再使用裸数字。所有请求返回相同的 `X-Trace-Id` 响应头和响应体 `traceId`。

原因：只用 HTTP 数字无法区分“设备不存在”和“日志不存在”，只判断中文消息又不稳定；同时保留两层标识可在不破坏现有调用方的情况下提供明确业务语义。真实 HTTP 状态也使代理、监控、Swagger 和普通 HTTP 客户端能正确识别失败。

日志边界：预期业务拒绝记录 WARN，内部业务操作失败与不可预期异常记录 ERROR 和服务端堆栈；日志统一包含事件名、错误码、HTTP状态、方法、路径和追踪号。请求体、密码、Token、MQTT 凭证不得进入异常日志，内部堆栈不得返回客户端。每秒轮询等正常请求不逐条记录访问日志，避免制造无用噪声。

## 0.10 自动化回归分层（2026-07-23）

最终选择：后端使用JUnit/Mockito覆盖MQTT、状态流转和HTTP端点；Flyway使用真实MySQL 8.4条件集成测试；前端使用与Vite配置兼容的Vitest测试纯TypeScript轮询控制器；Docker使用PowerShell静态约束加`docker compose config`验证。GitHub Actions组合执行全部层级。

原因：MQTT和状态机适合快速单元测试，数据库迁移必须使用真实MySQL语法验证，轮询生命周期需要可控假定时器，Docker升级首先要保证标签、远程模式和命名卷等不可破坏约束。分层后本地无需始终启动Broker、MySQL和完整容器，也能快速发现大多数回归；CI再补足真实MySQL。

边界：这些测试不替代群晖实机升级、真实HiveMQ和小智硬件验收。Flyway集成测试只操作随机生成的`aiot_regression_*`临时数据库并在结束后删除；部署检查不执行`down -v`、不读取私有环境文件，也不启动现有项目容器。
...
工具取舍：本阶段不采用Trivy Action，原因是当前日期可查到其2026-03供应链安全事件；选择Anchore官方Grype Action并固定到已核对的不可变提交。若后续更换扫描器，必须先更新问题记录、固定版本并让策略回归检查通过。

依赖修复策略：首次门禁发现Spring Boot 3.3.5默认的Tomcat 10.1.31存在已有修复版本的CRITICAL漏洞。当前先在同一Tomcat 10.1.x/Servlet 6.0兼容线显式升级到10.1.57，避免为单一容器依赖立即跨Spring Boot主维护线并连带改变Springdoc、MyBatis和Flyway行为。该覆盖必须由完整测试和镜像扫描共同验证；以后升级Spring Boot时应移除显式覆盖并重新使用其依赖管理。
...
国内仓库结论更新：按部署要求增加腾讯云TCR个人版作为国内同步源，固定路径为`ccr.ccs.tencentyun.com/aiot-log-system/aiot-log-backend`和`aiot-log-frontend`；GHCR继续作为唯一构建、漏洞门禁和来源证明主发布源。TCR工作流只能从GHCR复制已经发布的固定标签，不重新构建，不同步`latest`，并在发布后比较两个仓库的顶层digest；不一致必须失败。腾讯云命名空间需预先创建，用户名和登录密码只能保存在GitHub Actions Secrets，不进入仓库。若TCR个人版不支持完整复制带证明的OCI索引，则不得声称TCR自身具有独立签名；应通过相同digest关联GHCR来源证明。
...
## 0.12 设备中心与远程凭据页面（2026-07-24）

设备页面最终选择“显式展示模式”而不是根据自由文本设备类型或是否已有上报数据猜测。`monitoring_mode` 只有 `LOG_ONLY` 和 `TELEMETRY`，旧设备通过 Flyway V2 默认 `LOG_ONLY`。这样小智不会因偶发状态上报被错误展示成传感器，尚未产生数据的新传感器也可以预先配置数据页。

日志入口选择以设备工作台为主、全局日志路由兼容保留。设备日志组件始终固定 `deviceId` 并独立分页、筛选和自动刷新；采集模式复用现有 `device_reports` 和趋势组件。AI 只作为后续页签能力预留，本轮不引入模型调用或新结果表。

远程 HiveMQ 页面凭据选择宿主机私有文件持久化，不写 MySQL。原因是凭据属于部署秘密而非业务数据，已有 `docker/local` 就是私有配置边界；镜像更新时挂载目录可保留。接口不返回密码，保存使用临时文件加替换，Linux 尝试设置仅所有者读写，然后立即重建 Paho 连接。Broker 地址继续由部署环境控制，避免页面把连接目标改向非预期服务器。P1 登录与角色权限暂停期间，该能力只适用于可信网络，不允许公网开放。

并行实例边界：MQTT Client ID在同一Broker内必须唯一。本地远程源码Compose使用`aiot-log-backend-remote-local-source`，群晖GHCR/TCR成品Compose继续使用`aiot-log-backend-remote`，避免本地验收时把群晖连接踢下线。以后增加测试、预发布或第二台正式后端时，也必须分配独立Client ID。
...
`local_ai_discovery_failed`与`local_ai_fallback_to_official`表示系统已经选择可用的官方AI通道，业务能力仍然正常，因此统一记录为`INFO`、运行日志和已解决状态，不进入待处理报警。该规则以事件类型为准，可覆盖固件错误携带的`WARN/ERROR`等级。

边界：真实的本地AI连接失败、官方AI连接失败或协议错误仍按原始严重程度记录，不得为了减少告警而统一降级。
