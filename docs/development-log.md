...
补充（2026-08-02）：设备详情此前在一次通用刷新中串行查询设备、零件、MCP 操作记录和（采集设备的）遥测数据，使 MCP 操作结果可能被前置请求拖慢。现将设备与零件改为并行，MCP 记录拆为独立请求；切到 MCP 页签立即刷新，页签可见时按 500 ms 更新。该修复只影响管理页面显示速度；没有可优化官方 MCP、桥接器或小智固件实际执行耗时的证据。
