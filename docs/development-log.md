...
过程改进：本次升级先后遇到旧 Container Manager YAML 未更新、私有配置路径不一致、端口 `8080`/`80` 被占用、JWT 原因链缺失和压缩响应解析等可预见问题。后续不再依赖用户逐次截图猜测：所有群晖更新在部署前必须检查实际项目 YAML、镜像标签、宿主机端口、私有目录挂载及持久化文件；失败提示必须指出阶段与位置，后端异常保留安全 cause 链。该规则已进入项目设计，且仍禁止记录任何密钥、Token、密码或完整私有请求内容。

补充部署修复：群晖重新部署远程项目时返回 `Bind for 0.0.0.0:8080 failed: port is already allocated`。该端口是远程后端不必要的宿主机映射；前端本来通过 Docker 内部网络访问后端。现从所有远程 Compose 移除后端 `8080:8080` 映射，保留前端 `80`、MySQL 数据卷和内部健康检查；不需要删除数据卷或修改天气私有配置。

补充部署修复：移除后端端口冲突后，群晖进一步返回 `listen tcp4 0.0.0.0:80: bind: address already in use`，说明 NAS 自身服务占用了前端默认宿主机端口。远程 Compose 改为 `${WEB_PORT:-18080}:80`，容器内 Nginx 和 `/api` 代理保持不变；未设置 `WEB_PORT` 时通过 `http://NAS_IP:18080/` 访问，若该端口也被占用可只在私有环境文件设置另一个空闲端口。

补充诊断：专属 Host 的 DNS 与未认证 HTTPS 请求均可达，未认证请求返回预期 401，排除 Host、TLS 和公网连通性。原安全诊断只检查最外层异常消息；Java HTTP 客户端把 DNS、连接和超时等底层异常包装后会失去关键文本，从而回落为“请检查私有配置”。现改为安全遍历异常链，分别返回 Host 无法解析、请求超时、连接失败或 TLS 失败；未分类异常明确提示检查后端运行日志，不再误导为凭据配置问题。
...
验证：Vitest 覆盖“请求进行中不重叠、结束后 50 毫秒内恢复刷新”。环境监测完成后已统一构建离线镜像；当前应部署 `v1.4.0`，旧的中间包仅用于回退。仍需以真实小智 MQTT 启动事件测量 MQTT 入库至页面显示的端到端时间。

补充（2026-08-02）：设备详情此前在一次通用刷新中串行查询设备、零件、MCP 操作记录和（采集设备的）遥测数据，使 MCP 操作结果可能被前置请求拖慢。现将设备与零件改为并行，MCP 记录拆为独立请求；切到 MCP 页签立即刷新，页签可见时按 500 ms 更新。该修复只影响管理页面显示速度；没有可优化官方 MCP、桥接器或小智固件实际执行耗时的证据。新版小智同时发出 `startup` 与 `firmware_started` 时，旧的双边界策略会把同一次启动拆开；现仅 `startup` 开新批次，`firmware_started` 合并。离线包 `dist/aiot-remote-mqtt-v1.3.9-images.tar` 已构建，SHA-256 为 `AF87AB4BA931D53803995112F12D1D8ED6BC3BA49B487F2B6AE234C23CD5A12A`。同秒仍出现“1 条 + 3 条”时，原因是 `startup` 迟到抵达；补充源码以 2 秒窗口兼容这种乱序，尚未构建新包。

文案补充：新版小智上报 `Firmware startup completed`，此前未在中文映射表中而直接显示英文。现与历史 `Firmware initialization completed` 统一规范为“固件启动完成”；后端新入库记录和前端历史记录都适用。以上文案与 2 秒启动乱序兼容已构建到 `dist/aiot-remote-mqtt-v1.4.0-images.tar`，SHA-256 为 `B8FCFA8EEF25423305E52B8E2C8988356902252FE09DC5BCB54B5E6F701CF9D1`。

构建验证说明：开发机当前 Maven 进程使用 JRE，直接 `mvn test` 无法提供编译器；离线包改由 Docker 内 JDK 17 成功编译前后端、TypeScript 类型检查和生产构建。Dockerfile 的后端打包明确跳过测试，新增回归用例已编译，完整后端测试仍应在可用 JDK 17 的 CI 或开发环境执行。
...
## 2026-08-02 室外天气只有摘要信息

### 问题

- 环境监测页在天气请求成功后只显示天气现象、温湿度、AQI 和更新时间；用户无法查看风况、气压、污染物和健康建议。

### 根因

- 后端仅从和风实时天气和空气质量响应中提取摘要字段。`pm2p5` 虽预留在 V9 数据表和 API 对象中，但刷新逻辑没有赋值，前端也没有详情展示。

### 处理

- 新增 V11 扩展读数表，保存实时天气详情、常见污染物、空气质量等级、健康建议和监测站；空气质量请求增加中文参数，保持既有 gzip 解码和安全错误链。
- 前端保持摘要简洁，增加可展开详情；原始 ISO 时间统一格式化为本地秒级可读文本。所有新字段允许为空，因此供应商未提供的字段与历史读数不会引发刷新或渲染失败。

### 验证

- JDK 17 Maven 测试通过：42 项通过、0 失败、2 项独立 MySQL 迁移测试因未配置 `FLYWAY_TEST_DB_URL` 跳过；前端 TypeScript 检查和 Vite 生产构建通过。
- 离线镜像打包未执行：本机 Docker Desktop Linux 引擎未运行，Docker API 管道不存在。该环境问题与源码无关；必须在 Docker 引擎恢复后构建、校验 SHA-256 并更新版本记录，才能在群晖导入。部署时继续复用 MySQL 数据卷和 `docker/local` 私有配置。
