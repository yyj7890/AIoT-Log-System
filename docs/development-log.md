...
- 验证：前端 TypeScript 检查和 Vite 生产构建通过；仍需发布镜像后在浏览器中验证同页更新、跨页详情兜底、隐藏标签暂停和返回立即刷新。
...
- 验证：JDK 17 Maven 测试 12 项全部通过，包含新增标题/内容映射及原有远程 MQTT、`WARN`、去重、启动批次和状态流转回归。计划以独立候选版本 `v1.1.4-remote-mqtt` 发布，当前未提交、未打标签、未推送、未部署。
