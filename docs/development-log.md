...
- 发布：Flyway 实现已作为 `v1.1.8-remote-mqtt` 固定标签发布，前后端 GHCR 镜像和 GitHub Release 均由标签工作流生成；远程 GHCR Compose 已固定引用该版本。正在运行的群晖仍是 `v1.1.7-remote-mqtt`，数据库尚未改动；第一次部署 Flyway 镜像前仍需备份，并复用原数据卷和私有配置。
