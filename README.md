...
两个 GHCR 包同时保存局域网版和远程版：局域网版固定标签为 `v1.0.0-lan`，当前远程版为 `v1.1.3-remote-mqtt`，旧远程版 `v1.1.2-remote-mqtt`、`v1.1.1-remote-mqtt` 与 `v1.1.0-remote-mqtt` 继续保留用于回退。远程 GHCR Compose 只拉取当前远程固定标签；`latest` 保留给 `main` 的局域网版，且只有明确推送 `main` 分支时才允许更新。推送版本标签后，GitHub Actions 还会创建同名 Release，便于从仓库主页识别和进入远程版本。

```text
docker pull ghcr.io/yyj7890/aiot-log-backend:v1.1.3-remote-mqtt
docker pull ghcr.io/yyj7890/aiot-log-frontend:v1.1.3-remote-mqtt
```
