...
两个 GHCR 包同时保存局域网版和远程版：局域网版固定标签为 `v1.0.0-lan`，当前已发布远程版为 `v1.1.6-remote-mqtt`，增加仅限容器内部访问的后端健康检查，并让前端等待后端就绪后启动；旧远程版继续保留用于回退。`latest` 保留给 `main` 的局域网版，且只有明确推送 `main` 分支时才允许更新；远程版本标签不会覆盖它。

```text
docker pull ghcr.io/yyj7890/aiot-log-backend:v1.1.6-remote-mqtt
docker pull ghcr.io/yyj7890/aiot-log-frontend:v1.1.6-remote-mqtt
```
