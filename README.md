...
两个 GHCR 包同时保存局域网版和远程版：局域网版固定标签为 `v1.0.0-lan`，远程版固定标签为 `v1.1.0-remote-mqtt`。远程 GHCR Compose 只拉取远程固定标签；`latest` 保留给 `main` 的局域网版，且只有明确推送 `main` 分支时才允许更新。推送版本标签后，GitHub Actions 还会创建同名 Release，便于从仓库主页识别和进入远程版本。
