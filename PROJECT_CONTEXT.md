...
0. 将群晖升级到已发布的 `v1.1.5-remote-mqtt` 后，复现“首次日志请求失败/页面长时间挂起”场景，确认无需切换菜单也能自动恢复每秒刷新，并核对顶部系统运行时长在后端重启后归零。
