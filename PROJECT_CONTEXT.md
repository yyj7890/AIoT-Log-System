...
## 3. 已完成功能
- 2026-08-05 环境空间可绑定室内遥测设备和主小智；`xiaozhi-F02254` 可作为主动播报目标。规则新增室内温度、湿度、气压、光照，以及室外降水和天气文字变化；触发继续采用连续命中、冷却、静音和动态 TTS 保护。室内温度超阈值会建议开启空调或通风，降水提示携带雨具。Flyway V13 无损新增空间的室内传感器绑定；小智待机屏显示仍待确认实际板型后在固件项目实现。

- 2026-08-04 环境监测 ESP32 已通过 MQTT QoS 1 向 IoT 上报真实 BME280/BH1750 读数并收到 Broker PUBACK；IoT 遥测新增结构化 `pressure`（hPa）和 `illuminance`（lux），设备工作台以独立表格列和按实际有数据指标生成的趋势卡展示，避免将气压/光照塞入说明文本和展示空电压图。本机远程 Compose 仅用于构建和迁移验证，已成功执行 Flyway V12；用户已将 v1.4.1 包部署至 NAS 并烧录环境监测固件。`ENV-MONITOR-001` 已在本机验证库注册为 TELEMETRY；ESP-IDF 5.5.4 全量构建已通过，`Environment_Monitor/build/environment_monitor.bin` 为 `0xfe200`，3 MiB 应用分区剩余 67%，烧录不擦除 NVS。

- 2026-08-01 完成“环境监测”天气播报源码：环境空间保存地址、坐标和主小智，并可在页面编辑既有空间以补充坐标；私有天气服务刷新后保存室外天气/AQI读数。规则支持温度或 AQI 阈值/变化幅度、连续命中、冷却和跨午夜静音时段，并记录 `CLEAR`、`OBSERVED`、`SUPPRESSED`、`PUBLISHED`、`FAILED` 事件。只有私有动态 TTS 已启用、空间已启用且绑定有效小智时才允许投递，绝不以固定测试语音替代；未配置天气服务时页面明确显示未配置且不展示模拟数据。前端可在空间中新增、查看和删除规则；室内传感器仍未接入。
...
- 2026-08-01 已构建环境监测与日志刷新版本的私有远程离线镜像包 `dist/aiot-remote-mqtt-v1.2.8-images.tar`，SHA-256 为 `f595c27fc5e2bb578ee0e8a80babf541f051027c709eb9985fb516156b3a9884`。导入编排固定使用 `local/aiot-log-backend:v1.2.8-remote-mqtt` 与 `local/aiot-log-frontend:v1.2.8-remote-mqtt`；待人工导入群晖，必须继续复用现有远程项目名、MySQL 数据卷与 `docker/local/hivemq-remote.env`，包括私有天气配置与 PEM 文件。
- 2026-08-01 截图验收发现已建环境空间缺少坐标但无法编辑；已新增编辑入口并构建私有 `v1.2.9` 离线包（SHA-256 `0ebd377c551a491f1bd726bad0aae7991a64fba33cac99f6cb76a81cff0d3197`），供直接补填坐标后刷新真实天气。`v1.2.8` 保留回退。
- 2026-08-02 天气服务配置可直接在“环境监测”页面维护：Host、JWT kid、项目 ID 与首次/轮换 PEM 私钥由后端私有目录持久化，私钥不回显、不进数据库或 Git；远程 HiveMQ 编排已统一设置 `IOT_CONFIG_DIR=/app/private-config`，确保页面配置写入挂载的 `docker/local` 并跨容器重建保留。
- 2026-08-02 修复天气配置 Host 既可填写专属域名也可填写完整 HTTPS URL；后端请求前会安全补全缺失的 `https://`。
...
- 2026-08-02 已定位天气请求已认证成功但和风返回 gzip 压缩响应，旧客户端直接按 JSON 解析触发 `CTRL-CHAR`；已改为按响应编码自动解压后解析。
- 2026-08-02 已构建 `dist/aiot-remote-mqtt-v1.3.7-images.tar`（SHA-256 `579aa17e37656eb1de791c180e62890b4768dfe45461ad8e97dadbe2899f27f2`），用于兼容和风 gzip 响应。
- 2026-08-02 群晖宿主机 `8080` 已被占用；远程 Compose 已移除不必要的后端端口发布，前端改为继续仅通过 Docker 内部网络访问后端。
- 2026-08-02 群晖宿主机 `80` 已被占用；远程 Compose 的前端宿主机端口改为可覆盖的 `WEB_PORT`，默认 `18080`。
- 2026-08-02 项目维护规则补充：群晖更新前以 Container Manager 的项目 YAML 为准，预检镜像标签、宿主机端口、私有目录挂载和持久化文件；错误须直接给出脱敏的出错阶段与位置，后端保留安全 cause 链，禁止以逐次猜测代替预检。
- 2026-08-02 环境监测扩展为可展开的室外详情：刷新时保存实时天气的体感温度、风况、降水、气压、能见度、云量、露点，以及空气质量等级、PM2.5/PM10/NO₂/O₃/SO₂/CO、健康建议和关联监测站；摘要仍保留天气、温湿度和 AQI。历史旧读数详情字段为空时页面安全显示“−”，刷新后自动补齐；更新时间统一显示为本地可读的秒级格式。后端 42 项测试和前端生产构建已通过；`v1.3.8` 离线镜像包已在本机 Docker 引擎构建，待群晖导入验收。
- 2026-08-02 新版小智同时上报 `startup` 与 `firmware_started` 时，旧规则会将同一次启动拆成多条。现已改为仅 `startup` 开新批次，`firmware_started` 合并到该批次；另对迟到不超过 2 秒的 `startup` 事件合并当前批次，兼容新版固件的乱序上报，超过窗口仍视为真实重启。该补充修复和“固件启动完成”中文化已构建为 `dist/aiot-remote-mqtt-v1.4.0-images.tar`，SHA-256 为 `B8FCFA8EEF25423305E52B8E2C8988356902252FE09DC5BCB54B5E6F701CF9D1`；`v1.3.9` 与 `v1.3.8` 保留回退。
- `Firmware startup completed` 与旧文案 `Firmware initialization completed` 统一显示并写入为“固件启动完成”，不再保留英文。
- 2026-08-01 日志页面刷新收紧为 0.5 秒，并在慢请求结束后最多 50 毫秒补刷；已与环境监测一同构建为 `v1.2.8` 私有离线包。旧 `v1.2.7` 包保留用于回退；`v1.2.8` 尚待群晖导入和真实天气/小智播报验收。
- 2026-08-02 设备详情改为并行刷新设备档案和零件，AI MCP 操作记录不再被这些请求串行阻塞；切到 MCP 页签立即查询，并在该页签可见时每 0.5 秒刷新。该改动仅缩短操作结果在管理页的显示等待；官方 MCP/桥接器自身的执行耗时仍需按实际调用链单独定位。
...
- 连续运行事件汇总：同一设备在 30 秒内连续发布到 `/log` 的事件仅保留一条“设备运行上报（n 条）”日志，内容逐行保存每一步的最简状态；详情页以“固件开始初始化 → Wi-Fi 已连接 → …”展示，避免标题和消息重复。仅 `startup` 是新的启动批次边界；同一次启动中紧随的 `firmware_started` 会合并，避免新版小智将一次启动拆分为多条。其他事件超过窗口才新建日志。已由 `XIAOZHI-001` 实机验证 4 条启动事件成功合并，并验证电脑与小智同连手机热点时可自动发现 Broker 并正常上报；断网后可自动重连并恢复日志上传。

- 设备、日志、标签管理
- 日志状态、级别、类型、来源筛选
- 日志管理页和设备工作台日志在页面可见时每 0.5 秒拉取一次，切走页面或浏览器标签页隐藏时停止；自动请求不显示表格遮罩，使用请求互斥和禁止缓存参数，并按最近更新时间排序，使 30 秒汇总窗口内更新的运行日志也能及时回到顶部。若上一请求仍在进行，完成后最多 50 毫秒补发一次刷新，避免慢请求把延迟放大到数秒。详情弹窗打开时会同步当前页同 ID 的最新对象；记录不在当前页时使用详情接口静默兜底，避免重复并发，记录被删除时安全关闭弹窗
...
提醒管理页面私有离线 `v1.2.5-remote-mqtt` 镜像已构建并导出，待人工导入群晖：复用既有提醒查询/取消 API，不新增数据库或 MQTT 行为；提供全局筛选和设备详情页签，只允许取消尚未触发的 `SCHEDULED` 提醒。提醒仍必须由官方小智 MCP 创建，页面不绕过现有语音创建流程。

前端全页面自动刷新已完成源码实现：首页、设备、设备详情（含 AI MCP 操作）、提醒、告警规则、标签、MQTT 状态与设备日志均在页面可见时刷新；普通页面间隔为 5 秒，日志收紧为 0.5 秒。页面重新可见、窗口聚焦、网络恢复或浏览器恢复时立即刷新，后台不发起请求，并以请求互斥防止重复刷新；慢请求完成后最多 50 毫秒补发一次日志刷新。

该刷新改动的私有离线 `v1.2.6-remote-mqtt` 镜像已构建并导出，待人工导入群晖；`v1.2.5` 保留为仅含提醒管理页面的离线包。

项目主页 `README.md` 已同步记录动态 TTS、官方小智提醒、提醒管理页面和公开/私有离线镜像的版本边界；不包含私有地址、凭证、音频或设备数据。
