...
1. 群晖使用TCR固定镜像复用原数据卷和私有配置升级`v1.1.9`。
2. 按兼容规则演练回退`v1.1.8`，确认数据与服务恢复，再决定是否升回`v1.1.9`。
