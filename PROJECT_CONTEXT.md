...
- 2026-08-02 环境监测扩展为可展开的室外详情：刷新时保存实时天气的体感温度、风况、降水、气压、能见度、云量、露点，以及空气质量等级、PM2.5/PM10/NO₂/O₃/SO₂/CO、健康建议和关联监测站；摘要仍保留天气、温湿度和 AQI。历史旧读数详情字段为空时页面安全显示“−”，刷新后自动补齐；更新时间统一显示为本地可读的秒级格式。后端 42 项测试和前端生产构建已通过；`v1.3.8` 离线镜像包尚待本机 Docker 引擎启动后构建，不能误作已发布。
