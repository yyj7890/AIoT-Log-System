...
- 2026-08-01 完成“环境监测”天气播报源码：环境空间保存地址、坐标和主小智；私有天气服务刷新后保存室外天气/AQI读数。规则支持温度或 AQI 阈值/变化幅度、连续命中、冷却和跨午夜静音时段，并记录 `CLEAR`、`OBSERVED`、`SUPPRESSED`、`PUBLISHED`、`FAILED` 事件。只有私有动态 TTS 已启用、空间已启用且绑定有效小智时才允许投递，绝不以固定测试语音替代；未配置天气服务时页面明确显示未配置且不展示模拟数据。前端可在空间中新增、查看和删除规则；室内传感器仍未接入。
...
- 2026-08-01 日志页面刷新收紧为 0.5 秒，并在慢请求结束后最多 50 毫秒补刷；用户决定等待环境监测功能完成后再统一构建离线包。当前中间 tar 不作为部署包，旧 `v1.2.7` 包保留用于回退。
