...
2026-07-27 已构建并发布 IoT `v1.2.1-remote-mqtt`：后端包含固定 Opus 播报、提醒 API、MCP 审计和 Flyway V3～V6；前端包含 AI MCP 操作记录页。GHCR 前后端与 GitHub Release 已成功；腾讯云 TCR 前端同步成功，后端同步因仓库侧 HTTP/2 `PROTOCOL_ERROR` 失败，尚未可用于国内镜像部署。包尚未导入或部署到群晖。专用编排为 `docker-compose.remote.import.yml`，必须复用现有远程项目名、MySQL 卷和私有 `docker/local/hivemq-remote.env`。同日修正 Flyway 回归测试的过期表数断言，真实 MySQL 8.4 集成测试确认空库与既有 V1 库均可迁移至 V6。
